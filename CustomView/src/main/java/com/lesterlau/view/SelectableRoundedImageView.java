...
import com.lesterlau.view.blur.FastBlur;


/**
 * 带圆角的图片控件，可设置圆角的度数，边线的颜色、宽度，以及对该控件中的图片进行高斯模糊@Author lester
 *
 * @Author lester
 * @Date 2018/7/30
 */
        ...
        public void draw(Canvas canvas) {
            ...
            if (mOval) {
                ...
                if (mBorderWidth > 0) {
                    ...
                    mPath.addRoundRect(mBounds, mRadii, Path.Direction.CW);
//                    mBitmapPaint.setShadowLayer(5, 3, 3, 0xFFFF00FF);
                    canvas.drawPath(mPath, mBitmapPaint);
                }
            }
            canvas.restore();
    ...
    private float scaleFactor = 1;//bitmap在模糊处理时会先被缩小然后再放大,scaleFactor提供了需要缩小的等级.
    private float radius = 20;

    /**
     * 对图片添加高斯模糊效果
     *
     * @param scaleFactor 图片缩放的比率
     * @param radius      高斯模糊的比率
     */
    public SelectableRoundedImageView applyBlur(float scaleFactor, float radius) {
    ...
    /**
     * 对图片添加高斯模糊效果。默认不缩放，模糊比率20
     */
    public SelectableRoundedImageView applyBlur() {
     ...
     * @param view 当前控件
     */
    private void blur(Bitmap bkg, SelectableRoundedImageView view) {
