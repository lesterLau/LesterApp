...
public class TimePicker extends WheelPicker {
     ...
     */
    public TimePicker(FragmentActivity activity) {
     ...
     * @see #HOUR #HOUR#HOUR
     */
    public TimePicker(FragmentActivity activity, @Mode int mode) {
        super(activity);
        this.mode = mode;
        selectedHour = TimeUtil.fillZero(Calendar.getInstance().get(Calendar.HOUR_OF_DAY));
        selectedMinute = TimeUtil.fillZero(Calendar.getInstance().get(Calendar.MINUTE));
    }
    ...
    protected View makeCenterView() {
        ...
        for (int i = 0; i < 60; i++) {
            minutes.add(TimeUtil.fillZero(i));
        }
