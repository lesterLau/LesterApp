...
public final class ToastUtils {
     ...
     * @param text 文本
     */
    public static void showShortSafeForNotify(@NonNull final CharSequence text) {
        sHandler.post(new Runnable() {
            @Override
            public void run() {
                show(text, Toast.LENGTH_LONG, Gravity.TOP, 0, 0);
            }
        });
    }
    ...
    private static void show(final CharSequence text, final int duration, int mGravity, int mXOffset, int mYOffset) {
        cancel();
        boolean isCustom = false;
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
        ...
        sToast.setGravity(mGravity, mXOffset, mYOffset);
        sToast.show();
    }
